...
| Retrieve the current Artifact Signing profile root | **`artifact-signing-root`** (with **`--features artifact-signing-rest`**) — authenticated preview operation; validates the response as X.509 DER before writing it |
